 ...
 * — Redactor 9 compatible
 * — IE8 now compatible with json-encoded manifests
 * 
...
;(function($) {var _version = "jQuery.my 0.8.2";
				...
				$o.each(function() {
					...
					if ($this.is("div.redactor_box textarea")) {
                        ...
                        if(editor) {
                            ...
                            editor.bind("input keyup blur",(function($o){$o.trigger("redactor");}).fill($this));
                        }
                    }
					//ace fix
	...
	function xjs (node){ //recursively unwinds string def of funcs and regexps, modifies  source obj!
		for (var i in node) if (node.hasOwnProperty(i)) {
			...
			else if (t==St && /(^function\s\(|^new\sRegExp)/.test(n)) node[i]=eval('0||('+n+')'); 
			// Fucking IE, it took 8 hours to find this 0|| trick –
			// it allows IE8 eval functions as expected
		}
	}
